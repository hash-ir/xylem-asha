...
var encoderTestVectors = [
    ...
    null, "f6",
    undefined, "f7",
    new cbor_1.Simple(16), "f0",
    new cbor_1.Simple(24), "f818",
    new cbor_1.Simple(255), "f8ff",
    new Date("2013-03-21T20:04:00Z"), "c074323031332d30332d32315432303a30343a30305a",
    new cbor_1.Tagged(1, 1363896240), "c11a514b67b0",
    new cbor_1.Tagged(1, 1363896240.5), "c1fb41d452d9ec200000",
    new cbor_1.Tagged(23, new Uint8Array([1, 2, 3, 4])), "d74401020304",
    new cbor_1.Tagged(24, new Uint8Array([0x64, 0x49, 0x45, 0x54, 0x46])), "d818456449455446",
    new cbor_1.Tagged(32, "http://www.example.com"), "d82076687474703a2f2f7777772e6578616d706c652e636f6d",
    new Uint8Array(0), "40",
...
describe("cbor", function () {
    it("should encode and decode", function () {
        ...
        var encoded = cbor_1.encode(v);
        var decoded = cbor_1.decode(encoded);
        expect(decoded).toEqual(v);
    });
    it("should correctly encode test vectors", function () {
        for (var i = 0; i < encoderTestVectors.length; i += 2) {
            var value = encoderTestVectors[i];
            var expected = encoderTestVectors[i + 1];
            var got = cbor_1.encode(value);
            expect(hex.encode(got, true)).toBe(expected);
        }
    });
    it("should correctly decode test vectors", function () {
        for (var i = 0; i < decoderTestVectors.length; i += 2) {
            var expected = decoderTestVectors[i];
            var value = hex.decode(decoderTestVectors[i + 1]);
            var got = cbor_1.decode(value);
            expect(got).toEqual(expected);
        }
    });
    it("should encode and decode -0.0", function () {
        var encoded = cbor_1.encode(-0.0);
        var decoded = cbor_1.decode(encoded);
        expect(1 / decoded).toBe(-Infinity);
    });
    it("should throw if 64-bit integer is too large", function () {
        var vectors = [
            "1bffffffffffffffff",
            "3bffffffffffffffff",
        ];
        vectors.forEach(function (v) {
            expect(function () { return cbor_1.decode(hex.decode(v)); }).toThrowError(/too large/);
        });
    });
    it("should throw if there's extra data and the end", function () {
        var encoded = bytes_1.concat(cbor_1.encode("Hello world"), new Uint8Array(1));
        expect(function () { return cbor_1.decode(encoded); }).toThrowError(/extra/);
    });
    it("should throw for objects cycles", function () {
        var a = { "one": 1 };
        var b = { "two": 2 };
        // create cycle
        b["a"] = a;
        ...
        expect(function () { return cbor_1.encode(a); }).toThrowError(/call stack/);
        expect(function () { return cbor_1.encode(["insideArray", a]); }).toThrowError(/call stack/);
        expect(function () { return cbor_1.encode({ "insideObject": a }); }).toThrowError(/call stack/);
    });
    it("should encode maps with integer keys differently from string keys", function () {
        var a = { 1: "one", 2: "two", "3str": "three" };
        var eai = cbor_1.encode(a, { intKeys: true });
        // make sure it decodes
        var dec = cbor_1.decode(eai);
        expect(dec).toEqual(a);
        // make sure it differs from string encoding
        expect(cbor_1.encode(a)).not.toEqual(eai);
    });
    it("should encode and decode custom tagged object", function () {
        ...
        var HelloEncoder = function (h) {
            if (!(h instanceof Hello)) {
                return undefined;
            }
            return new cbor_1.Tagged(7777, h.get());
        };
        var HelloDecoder = function (_a) {
        ...
        var encodedData = cbor_1.encode(testData, {
            taggedEncoders: cbor_1.DEFAULT_TAGGED_ENCODERS.concat(HelloEncoder)
        });
        var decodedData = cbor_1.decode(encodedData, {
            taggedDecoders: cbor_1.DEFAULT_TAGGED_DECODERS.concat(HelloDecoder)
        });
