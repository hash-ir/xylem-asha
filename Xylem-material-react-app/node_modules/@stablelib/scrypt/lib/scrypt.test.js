...
describe("scrypt.deriveKey", function () {
    it("should derive correct key", function () {
        vectors.forEach(function (v) {
            var password = utf8_1.encode(v.password);
            var salt = utf8_1.encode(v.salt);
            var key = scrypt_1.deriveKey(password, salt, 1 << v.logN, v.r, v.p, v.dkLen);
            expect(base64_1.encode(key)).toBe(v.result);
        });
    });
    it("should derive correct key (non-blocking)", function (done) {
        function runTests(i) {
            if (i === vectors.length) {
                done();
                return;
            ...
            var password = utf8_1.encode(v.password);
            var salt = utf8_1.encode(v.salt);
            scrypt_1.deriveKeyNonBlocking(password, salt, 1 << v.logN, v.r, v.p, v.dkLen).then(function (key) {
                expect(base64_1.encode(key)).toBe(v.result);
                runTests(i + 1);
            });
        }
